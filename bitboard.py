...
def init_tables():
    ...
    for sq in range(64):
        ...
        if r < 7:
            if f > 0: pawn_attacks[WHITE][sq] |= (np.uint64(1) << np.uint64((r+1)*8 + f-1))
            if f < 7: pawn_attacks[WHITE][sq] |= (np.uint64(1) << np.uint64((r+1)*8 + f+1))
        if r > 0:
            if f > 0: pawn_attacks[BLACK][sq] |= (np.uint64(1) << np.uint64((r-1)*8 + f-1))
            if f < 7: pawn_attacks[BLACK][sq] |= (np.uint64(1) << np.uint64((r-1)*8 + f+1))
...
def encode_move(fr, to, promo, piece_moved, captured, ep, castle):
    return np.uint32(fr | (to << 6) | (promo << 12) | (piece_moved << 15) | (captured << 18) | (ep << 21) | (castle << 22))
...
def is_square_attacked(sq, color, pieces, colors):
    ...
    if color == WHITE:
        if PAWN_ATTACKS[BLACK][sq] & pieces[PAWN] & colors[WHITE]: return True
    else:
        if PAWN_ATTACKS[WHITE][sq] & pieces[PAWN] & colors[BLACK]: return True
            
    if KNIGHT_ATTACKS[sq] & pieces[KNIGHT] & colors[color]: return True
    if KING_ATTACKS[sq] & pieces[KING] & colors[color]: return True
        
    b_attacks = get_bishop_attacks(sq, blockers)
    if b_attacks & (pieces[BISHOP] | pieces[QUEEN]) & colors[color]: return True
        
    r_attacks = get_rook_attacks(sq, blockers)
    if r_attacks & (pieces[ROOK] | pieces[QUEEN]) & colors[color]: return True
            ...
            if pt == PAWN:
                if turn == WHITE:
                    ...
                    if (all_pieces & (np.uint64(1) << np.uint64(to))) == 0:
                        promo = (to >= 56)
                        if promo:
                            moves[count] = encode_move(sq, to, QUEEN, PAWN, NONE, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, ROOK, PAWN, NONE, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, BISHOP, PAWN, NONE, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, KNIGHT, PAWN, NONE, 0, 0); count += 1
                        else:
                            moves[count] = encode_move(sq, to, NONE, PAWN, NONE, 0, 0); count += 1
                            if sq < 16:
                                to2 = sq + 16
                                if (all_pieces & (np.uint64(1) << np.uint64(to2))) == 0:
                                    moves[count] = encode_move(sq, to2, NONE, PAWN, NONE, 0, 0); count += 1
                    ...
                    while attacks:
                        ...
                        promo = (to >= 56)
                        if promo:
                            moves[count] = encode_move(sq, to, QUEEN, PAWN, cap, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, ROOK, PAWN, cap, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, BISHOP, PAWN, cap, 0, 0); count += 1
                            moves[count] = encode_move(sq, to, KNIGHT, PAWN, cap, 0, 0); count += 1
                        else:
                            moves[count] = encode_move(sq, to, NONE, PAWN, cap, 0, 0); count += 1
                            
                    if ep_square != 64:
                        if PAWN_ATTACKS[WHITE][sq] & (np.uint64(1) << np.uint64(ep_square)):
                            moves[count] = encode_move(sq, ep_square, NONE, PAWN, PAWN, 1, 0); count += 1
                else:
                    to = sq - 8
                    if (all_pieces & (np.uint64(1) << np.uint64(to))) == 0:
                        promo = (to < 8)
                ...
                elif pt == BISHOP: attacks = get_bishop_attacks(sq, all_pieces)
                elif pt == ROOK: attacks = get_rook_attacks(sq, all_pieces)
                elif pt == QUEEN: attacks = get_queen_attacks(sq, all_pieces)
                elif pt == KING: attacks = KING_ATTACKS[sq]
                    ...
                    if (np.uint64(1) << np.uint64(to)) & them_pieces:
                        for cpt in range(6):
                            if pieces[cpt] & (np.uint64(1) << np.uint64(to)):
                                cap = cpt
                                break
                    moves[count] = encode_move(sq, to, NONE, pt, cap, 0, 0); count += 1
                    
    castling = state[1]
    if turn == WHITE:
        if (castling & CR_WK) and not (all_pieces & np.uint64(0x60)) and not is_square_attacked(4, BLACK, pieces, colors) and not is_square_attacked(5, BLACK, pieces, colors):
            moves[count] = encode_move(4, 6, NONE, KING, NONE, 0, 1); count += 1
        if (castling & CR_WQ) and not (all_pieces & np.uint64(0xE)) and not is_square_attacked(4, BLACK, pieces, colors) and not is_square_attacked(3, BLACK, pieces, colors):
            moves[count] = encode_move(4, 2, NONE, KING, NONE, 0, 1); count += 1
    else:
        if (castling & CR_BK) and not (all_pieces & np.uint64(0x6000000000000000)) and not is_square_attacked(60, WHITE, pieces, colors) and not is_square_attacked(61, WHITE, pieces, colors):
            moves[count] = encode_move(60, 62, NONE, KING, NONE, 0, 1); count += 1
        if (castling & CR_BQ) and not (all_pieces & np.uint64(0x0E00000000000000)) and not is_square_attacked(60, WHITE, pieces, colors) and not is_square_attacked(59, WHITE, pieces, colors):
            moves[count] = encode_move(60, 58, NONE, KING, NONE, 0, 1); count += 1
    ...
    if state[1]:
        if piece_moved == KING:
            if turn == WHITE:
                ...
                state[1] &= ~(CR_BK | CR_BQ)
        if fr == 0 or to == 0: state[1] &= ~CR_WQ
        if fr == 7 or to == 7: state[1] &= ~CR_WK
        if fr == 56 or to == 56: state[1] &= ~CR_BQ
        if fr == 63 or to == 63: state[1] &= ~CR_BK
        
    king_sq = lsb(pieces[KING] & colors[turn])
    if is_square_attacked(king_sq, opp, pieces, colors):
        return False
    return True
...
def from_chess_board(board):
    ...
    c = 0
    if board.has_kingside_castling_rights(chess.WHITE): c |= CR_WK
    if board.has_queenside_castling_rights(chess.WHITE): c |= CR_WQ
    if board.has_kingside_castling_rights(chess.BLACK): c |= CR_BK
    if board.has_queenside_castling_rights(chess.BLACK): c |= CR_BQ
    state[1] = c
...
def decode_move(move):
    ...
    promo_str = ""
    if promo == KNIGHT: promo_str = "n"
    elif promo == BISHOP: promo_str = "b"
    elif promo == ROOK: promo_str = "r"
    elif promo == QUEEN: promo_str = "q"
