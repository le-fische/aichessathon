...
def qsearch(pieces, colors, state, alpha, beta, ply, path_keys, path_count, start_time, budget_ms, nodes):
    ...
    # CANDIDATE FIX (tools/scratch only, NOT applied to the shipping file).
    # The shipped qsearch takes start_time and budget_ms and never reads either:
    # the only abort check is in negamax, so a large qsearch subtree runs past the
    # deadline unchecked. Same cadence and same deadline as negamax.
    if (nodes[0] & 255) == 0:
