...
measure() {
    echo "Measuring $1"
    for i in 0 1 2 3; do
        nps=$(uv run python tools/measure_version.py . "${FENS[$i]}" 3000 | grep NPS | awk '{print $2}')
        echo "FEN $((i+1)): $nps"
    done
}

# Current state (All Speedups)
...
# Revert Speedup 1 (hash)
cat << 'PYEOF' > patch.py
with open("search.py", "r") as f:
    c = f.read()
c = c.replace("import chess\n", "import chess\nimport chess.polyglot\n")
c = c.replace("hash_key = ctx.board._transposition_key()", "hash_key = chess.polyglot.zobrist_hash(ctx.board)")
with open("search.py", "w") as f:
    f.write(c)
PYEOF
uv run python patch.py
measure "Baseline (No Speedups)"
