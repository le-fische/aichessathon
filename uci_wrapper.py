#!/usr/bin/env python3
"""
A simple UCI wrapper to allow plugging the agent into standard chess GUIs
(like BanksiaGUI, Scid vs Mac, Arena, En Croissant).
"""
import sys
import os

# Add the current directory to sys.path so it can import agent
sys.path.insert(0, os.path.abspath(os.path.dirname(__file__)))

from agent import get_move

def main():
    # Make sure we don't buffer output
    # sys.stdout.reconfigure(line_buffering=True) is useful, or we can explicitly flush
    ...
    while True:
        ...
        if cmd == "uci":
            ...
            # Extract wtime/btime based on whose turn it is
            try:
                import chess
                board = chess.Board(fen)
